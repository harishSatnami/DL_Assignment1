...
def gradient_descent(input, learning_rate, number_of_layers, max_iteration, nodes_per_hidden_layer, nodes_in_output_layer):
    Weights, Biases = random_initialize(number_of_layers,nodes_per_hidden_layer,nodes_in_output_layer)
    itr = 0
    while itr<max_iteration:
        H, A, y_pred = forward_propagation(input, Weights, Biases, number_of_layers)
        delta_Weights, delta_Biases = backward_propagation(H, A, Weights, y_actual, y_pred, number_of_layers)
